...
- 普通品质1/2无技能道具按210ms节奏完整出现，有有效类型1技能时先保留真实名称300ms。品质3出场800ms（480ms起图标），品质4/5出场1100ms（780ms起图标），图标320ms落位后都额外保留真实名称/基础数量300ms再斧技或下一件；同一道具连续技能不重复停留。出场只读取道具品质，技能文字与数量色只读取实际BUFF品质。
- 出场复用V7墨团；先墨记后图标，高阶仅增加同图淡叠层，不扩大格子或添加光圈。新出现音rewardRare/rewardHigh分别0.8/1.1秒，图标回弹重音约640/940ms；不同时叠普通rewardReveal，场景落物音不变。
