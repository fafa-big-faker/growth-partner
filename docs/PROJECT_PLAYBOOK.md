...
- `boot-assets.js`是生成的精确URL/字节/SHA256清单，包含公共图像、九把斧头90帧和11音频。准备页背景和进度素材优先，DPR只下载对应一套树图。`ResourcePack`最多6并发、校验完整字节、失败仅重试缺项。90帧和各大场景先下载，不同时全解码；登录后只解码当前修仙背景、当前树及其亮层、当前武器10帧。`AssetPreloader`共享解码和在途URL，取消只释放当前消费者。
