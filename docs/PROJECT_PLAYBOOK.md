...
- 仙斧评级调用`WeaponAffixes.getWeaponRating(weapon)`：只读该实例保存的`skillRolls`，有效技能的`buffQuality`1至5对应B/A/S/SS/SSS，多技能取最高，无技能为B。不要用斧头品质、BUFF组ID或奖励目标品质代替。正式背包字标尚待用户生成，提示词在`docs/art-prompts/仙来-仙斧评级字标.md`，图片到位前不以临时字标冒充接入完成。
