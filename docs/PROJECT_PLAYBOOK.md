...
- 登录出场只播一次，结束后 Logo 以 6.4 秒周期轻浮 10px；不要让表单跟着移动。湖面只绘制少量横向水纹和两组扁平涟漪，不移动整张背景。
- `login-art.js` 的湖面遮罩以 1448x1086 原图坐标定义，并与背景 `cover` 裁切同步。更换背景时必须重新核对水域遮罩；不能直接用屏幕高度百分比放波纹，以免手机上漂到山石或输入框。
...
- 动效测试不能只判断Canvas存在非零像素。水纹需要检查实际可见范围与足够的不透明度，涟漪中心在背景水域和屏幕的交集内选取，并避开完整表单。原版1px/低透明度且大片被避让区裁掉，虽然在动但玩家看不见。
