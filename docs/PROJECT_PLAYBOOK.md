...
- 锻造结果按斧头框、名称、品质、技能、装备文案、材料、再次锻造、折叠概率的顺序展示。名称下方固定160px信息区在抽取时显示状态/进度，结果时隐藏状态/进度并替换为技能文案，不能再追加到按钮后面。
- 斧头框144x174px，短屏128x154px，框内装备操作仍为44px。长技能在信息区内滚动，短横屏主内容可滚动，材料与主按钮不被挤走。无技能显示“暂无斧技”；再次锻造清除旧技能和装备操作并重置内容滚动，不改变2至3秒、前60%推进进度/后40%高速轮播的原时间轴。
...
- 正式字标用`renderWeaponRating(weapon, context)`统一接入：武器库格底、锻造结果品质旁、当前装备品质旁。格底36x18px，行内48x24px，图片自身保留透明边和统一字高；不加徽章底板或放大格子。当前状态改右上Lucide对勾，锁与“新”在顶部状态区分行；手机品质/评级一行放到斧图和名称下，防止窄列挤压。仅图片失败时用真实评级文字同尺寸兜底，不改变占位。
- 换装/背包重绘时评级读取当前UUID实例，同道具ID的不同副本不能共用评级；保留两栏滚动和武器库模式。锻造成功显示评级，下一次抽取清除；不改变原有160px信息区与锻造时间轴。`tests/weapon-rating.browser-check.cjs`检查320/360/390/1440视口、实际图片解码、五评级、混合角标、换装/滚动、结果切换和图片失败兜底，无截图/真实数据写入。
