...
- 新五阶段宝树使用同一384x384透明画布，受击点为图像内176/384、272/384；树冠落叶锚点按各树原画测量，保存在外观描述及资产manifest。缺少元数据的历史目标才回退树框42%/45%。所有位置从实际显示坐标换回场景坐标，不能把缩放后的屏幕坐标当成本地像素。
...
- 原图导入和指纹见`ASSET_PIPELINE.md`第18节。网页十张树图/亮层及预载使用相同`wish-trees-20260909`版本。`tests/wish-trees.browser-check.cjs`用模拟数据检查五视口五皮肤、真实图像/透明边、统一锚点、树冠/主干点击、升级提示与动效清理，不截图或修改玩家数据。
