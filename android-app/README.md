...
当前交付1.0.3（versionCode 4）。原包名、签名和本地存储不变，可直接覆盖安装1.0.2，无需卸载。APK 内置构建时 `BootAssetManifest` 中全部精确资源；线上 URL、字节数和 SHA-256 都匹配时由本地直接提供，任一项变化则自动回到网络，因此网页功能、配置和新版美术仍可正常热更新。普通浏览器登录前只准备 `phase=boot` 的公共资源，未装备的90帧仙斧动作和未使用仙树延后到实际需要时加载。

1.0.3 的12个短音效通过受限的原生 `SoundPool` 播放，BGM继续由网页单实例播放。两个网页接口仅向可信游戏页提供固定音效播放/停止和内置资源精确匹配查询，不提供文件、网址或网络访问能力。启动仍直接交给网页入境准备；原生面板只在真正的网络/引擎错误时显示。
