...
当前交付1.0.4（versionCode 5）。原包名、签名和本地存储不变，可直接覆盖安装1.0.3，无需卸载。APK 内置构建时 `BootAssetManifest` 中全部精确资源；线上 URL、字节数和 SHA-256 都匹配时由本地直接提供，任一项变化则自动回到网络，因此网页功能、配置和新版美术仍可正常热更新。普通浏览器登录前只准备 `phase=boot` 的公共资源，未装备的90帧仙斧动作和未使用仙树延后到实际需要时加载。

1.0.4 的12个短音效在内存中预解码并转换到手机报告的最佳输出采样率，通过持续预热的低延迟 `AudioTrack` 混音播放；切到后台立即停止音轨，回到前台重新预热。BGM继续由网页单实例播放。两个网页接口仅向可信游戏页提供固定音效播放/停止、播放后端标识和内置资源精确匹配查询，不提供文件、网址或网络访问能力。原生音轨初始化失败时短音效自动回退现有 Web Audio。启动仍直接交给网页入境准备；原生面板只在真正的网络/引擎错误时显示。
