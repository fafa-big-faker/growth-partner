...
def build_v6_assets() -> tuple[dict, int, int, int]:
    source_manifest = SOURCE_ROOT / "v6/source-manifest.json"
    if not source_manifest.exists():
        return {}, 0, 0, 0
    specs = json.loads(source_manifest.read_text(encoding="utf-8"))["assets"]
    manifest = {}
    ...
    for group, assets in specs.items():
        ...
        for name, spec in assets.items():
            source = ROOT / spec["path"]
            destination = RUNTIME_ROOT / "v6" / group / f"{name}.webp"
            limit = (48, 112) if group == "quality" else ((960, 256) if name == "login-brush" else (768, 256))
            with Image.open(source) as image:
                optimized = fit_within(image, limit)
                save_webp(optimized, destination, quality=94 if group == "quality" else 88)
                manifest[group][name] = {
                    "path": destination.relative_to(ROOT).as_posix(),
                    "size": list(optimized.size),
                    "alphaRange": list(optimized.getchannel("A").getextrema()),
            ...
            runtime_bytes += destination.stat().st_size
            count += 1
    destination = RUNTIME_ROOT / "v6/manifest.json"
    destination.parent.mkdir(parents=True, exist_ok=True)
    destination.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return manifest, count, source_bytes, runtime_bytes
...
def main() -> None:
    ...
    if args.v6_only:
        ...
        if not count:
            raise FileNotFoundError("Import V6 artwork before building its runtime assets")
        print(f"V6 assets: {count} ({mib(source_bytes):.2f} MiB -> {mib(runtime_bytes):.2f} MiB)")
        return
    if args.v5_only:
